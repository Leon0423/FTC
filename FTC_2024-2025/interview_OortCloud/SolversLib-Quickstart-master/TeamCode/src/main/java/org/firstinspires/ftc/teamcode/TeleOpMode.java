...
public class TeleOpMode extends LinearOpMode {
    ...
    private static final double ARM_STEP = 0.005;
    private static final double WRIST_STEP = 0.005;
    private double ArmInitialPosition = 0.20;
    private double armAngle = ArmInitialPosition;
    private double wristOffset = 0.0;
    private double armLeftTarget = ArmInitialPosition;
    private double armRightTarget = ArmInitialPosition;
    ...
    public void runOpMode() throws InterruptedException {
        ...
        while(opModeIsActive()) {
            ...
            // * Arm differential: average angle + wrist offset
            // * gamepad2.x / y: armAngle, gamepad2.left_bumper / right_bumper: wristOffset
            // * --------------------------------------------------- //
            if(gamepad2.x) {
                armAngle += ARM_STEP;
            }
            ...
            if(gamepad2.right_bumper) {
                wristOffset += WRIST_STEP;
            }
            if(gamepad2.left_bumper) {
                wristOffset -= WRIST_STEP;
            }

            armAngle = clamp(armAngle, ARM_SERVO_MIN, ARM_SERVO_MAX);
            ...
            // * Arm
            telemetry.addLine("Arm: x/y angle, left/right bumper wrist");
    ...
    public void init_hardware() {
        ...
        armAngle = clamp(ArmInitialPosition, ARM_SERVO_MIN, ARM_SERVO_MAX);
        armLeftTarget = clamp(armAngle + wristOffset, ARM_SERVO_MIN, ARM_SERVO_MAX);
        armRightTarget = clamp(armAngle - wristOffset, ARM_SERVO_MIN, ARM_SERVO_MAX);
        ArmLeft.setPosition(armLeftTarget);
        ArmRight.setPosition(armRightTarget);
