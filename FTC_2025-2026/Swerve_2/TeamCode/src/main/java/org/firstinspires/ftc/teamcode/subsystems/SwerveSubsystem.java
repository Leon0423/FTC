...
public class SwerveSubsystem extends SubsystemBase {
    ...
    public void zeroHeading() {
        headingOffsetDeg = imu.getRobotYawPitchRollAngles().getYaw(AngleUnit.DEGREES);
    }

    public double getHeading() {
        double raw = imu.getRobotYawPitchRollAngles().getYaw(AngleUnit.DEGREES);
        double heading = raw - headingOffsetDeg;
    ...
    // ── Offset (SharedPreferences) ──────────────────────────────────────

    public void loadOffsets(SharedPreferences offsetPrefs) {
        frontLeft.loadOffsetFromPrefs(offsetPrefs);
        frontRight.loadOffsetFromPrefs(offsetPrefs);
        backLeft.loadOffsetFromPrefs(offsetPrefs);
        backRight.loadOffsetFromPrefs(offsetPrefs);
    }

    public boolean hasStoredOffsets(SharedPreferences offsetPrefs) {
        return frontLeft.hasStoredOffset(offsetPrefs)
                && frontRight.hasStoredOffset(offsetPrefs)
                && backLeft.hasStoredOffset(offsetPrefs)
                && backRight.hasStoredOffset(offsetPrefs);
    }

    public void initializeAllFromAbsolute() {
        frontLeft.initializeTrackingFromAbsolute();
        frontRight.initializeTrackingFromAbsolute();
        backLeft.initializeTrackingFromAbsolute();
        backRight.initializeTrackingFromAbsolute();
    }
