...
public class SwerveModule {
    ...
    public String getTurningServoName() {
        return turningServoName;
    }

    public void loadOffsetFromPrefs(SharedPreferences prefs) {
        if (prefs.contains("offset_" + turningServoName)) {
            absoluteEncoderOffsetRad = prefs.getFloat(
                    "offset_" + turningServoName, (float) absoluteEncoderOffsetRad);
        }
    }

    public boolean hasStoredOffset(SharedPreferences prefs) {
        return prefs.contains("offset_" + turningServoName);
    }

    public void initializeTrackingFromAbsolute() {
        lastRawAngleRad = getRawServoRadians();
        double orientedRaw = turningEncoderReversed ? -lastRawAngleRad : lastRawAngleRad;
        double wheelAngleRad = orientedRaw * ModuleConstants.kTurningMotorGearRatio
                - absoluteEncoderOffsetRad;
        accumulatedWheelDeg = Math.toDegrees(angleWrap(wheelAngleRad));
