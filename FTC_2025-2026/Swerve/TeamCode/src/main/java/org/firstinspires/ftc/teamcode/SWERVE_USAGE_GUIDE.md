...
> **文件定位：**
> 這份文件整合了硬體配置、調校流程、正式操作與疑難排查。  
> 第一次上車建議先看「快速開始」，再依序執行完整流程。

---

## 快速開始

1. 完成 Driver Station 硬體命名（先看 [2. 首次安裝與硬體配置](#2-首次安裝與硬體配置)）。
2. 依序跑 Tuning（[4. Tuning 程式流程（必照順序執行）](#4-tuning-程式流程必照順序執行)）。
3. 用 `Swerve_Control` 實際駕駛（[5. 正式操作：Swerve_Control](#5-正式操作-swerve_control)）。
4. 覺得不穩定時先查 [10. 常見問題排查](#10-常見問題排查)。
...
> **重點：** hardwareMap 命名會對應車頭位置，請同步修改 `Constants.java` 中對應字串常數。
...
2. 連線後以瀏覽器開啟 `http://192.168.43.1:8080/dash`（後續需調整 PID 時再打開）
...
> **提示：** 按 `A` 會同時清除累積角追蹤記憶，並覆蓋既有 Offset。
...
> **注意：** Dashboard 調整只在本次執行期有效，滿意後請手動更新 `Constants.java`。
